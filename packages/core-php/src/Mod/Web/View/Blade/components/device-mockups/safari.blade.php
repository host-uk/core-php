{{--
    ...
    @props
    - width: int (default: 1203) - Width of the SVG
    - height: int (default: 753) - Height of the SVG
    - url: string|null - URL to display in the address bar
    - src: string|null - Optional image URL to display in the browser window

    Usage:
    <x-device-mockups.safari url="https://host.uk.com">
        <!-- Slot content renders inside the browser window -->
    </x-device-mockups.safari>

    Or with an image:
    <x-device-mockups.safari url="https://host.uk.com" src="/path/to/screenshot.png" />
--}}
...
@php
    ...
    $pathId = 'safari-path-' . uniqid();
    $clipId = 'safari-clip-' . uniqid();
@endphp
...
<div class="relative" style="width: {{ $width }}px;">
    ...
    >
        <g clip-path="url(#{{ $pathId }})">
            {{-- Browser content area background --}}
            <path
        ...
        <defs>
            <clipPath id="{{ $pathId }}">
                <rect fill="white" width="1203" height="753" />
            </clipPath>
            <clipPath id="{{ $clipId }}">
                <path
    ...
    {{-- Slot content overlaid on browser viewport area (for interactive/dynamic content) --}}
    @if($slot->isNotEmpty())
        <div
            class="absolute overflow-hidden"
            style="
                top: {{ ($viewportY / 753) * 100 }}%;
                left: {{ (1 / 1203) * 100 }}%;
                width: {{ (1200 / 1203) * 100 }}%;
                height: {{ ($viewportHeight / 753) * 100 }}%;
                border-bottom-left-radius: {{ 12 * $scale }}px;
                border-bottom-right-radius: {{ 12 * $scale }}px;
            "
        >
            {{ $slot }}
        </div>
